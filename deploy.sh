...
helm upgrade --kube-context $1 -f apm-values.yaml --create-namespace -n centralized-logging -i --wait --timeout 20s apm-server elastic/apm-server
helm upgrade --kube-context $1 --create-namespace -n centralized-logging -i --wait --timeout 20s metricbeat elastic/metricbeat
